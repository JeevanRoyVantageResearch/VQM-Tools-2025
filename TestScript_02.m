...
hardcodingDataFile = 'Hardcoding - info - Updated.xlsx'; % Replace with the actual file name
...
dataFile = params.pkDataFilePath; % Replace with the actual file name-----------------------------------------------> Input1
%dataFile = readtable("asdfghjkl") ;
try
...
response_species_compartment = 'Cen';%-----------------------
response_species = 'DrugConc_Cen_ugmL'; %----------------------------
...
switchValues = paramsToBeEstimatedDataTable.Switch;
...
% Filter only the parameters with switchValues == 1
activeIndices = switchValues == 1;
filteredParamNames = paramNames(activeIndices);
filteredTransformations = transformations(activeIndices);
filteredInitialValues = initialValues(activeIndices);
filteredLowerBounds = lowerBounds(activeIndices);
filteredUpperBounds = upperBounds(activeIndices);

% Create estimatedInfo object dynamically
estimatedInfoObj = estimatedInfo(strcat(filteredTransformations, '(',filteredParamNames, ')'));


% Assign initial values and bounds
for i = 1:length(filteredParamNames)
    estimatedInfoObj(i).InitialValue = filteredInitialValues(i);
    estimatedInfoObj(i).Bounds = [filteredLowerBounds(i), filteredUpperBounds(i)];
end
...
doses1.TargetName = 'Cen.Drug_Cen_mg';
...
f.Data         = groupedDataObj; %------------> PK DATA
f.Model        = model; %---------------------> MODEL
f.Estimated    = estimatedInfoObj; %----------> ESTIMATE_PARAMS DATA
f.ResponseMap  = responseMap; %---------------> PK_DATA_COL = MODEL_SPECIES
f.ErrorModel   = 'exponential'; %-------------> ERROR MODEL
f.Doses        = dosesForFit; %---------------> DOSES
f.FunctionName = 'particleswarm'; %-----------> ESTIMATION METHOD
f.Options      = options; %-------------------> OPTIONS
f.ProgressPlot = true; %----------------------> [???]
f.UseParallel  = true; %----------------------> PARALLEL COMPUTING ENABLED
f.Pooled       = true; %----------------------> POOLED ENABLED
